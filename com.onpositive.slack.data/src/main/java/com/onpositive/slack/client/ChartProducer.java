...
public class ChartProducer implements IContentProducer{
	...
	public byte[] content() {
		...
		BufferedImage createBufferedImage = histogram.createBufferedImage(700, 500);
		ImageWriter next = ImageIO.getImageWritersByFormatName("png").next();		
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		try {
			next.setOutput(ImageIO.createImageOutputStream(baos));
		} catch (IOException e) {
			e.printStackTrace();
		}
		try {
			next.write(createBufferedImage);
		} catch (IOException e) {
			e.printStackTrace();
		}
